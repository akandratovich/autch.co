...
public class AacToMpegStreamCodec extends StreamCodec {
  private final MediaPacket   readPacket    = MediaPacket.make(10240);
  private final MediaPacket   writePacket   = MediaPacket.make(10240);
  
  private final DemuxerFormat demuxerFormat = DemuxerFormat.findFormat("adts");
  
  public AacToMpegStreamCodec(String stream) {
    super(stream, MuxerFormat.guessFormat("mp3", null, "audio/mpeg"));
  }
  ...
  public void decode(Path source) throws Exception {
    ...
    try (StreamResampler resampler = new StreamResampler(decoder, encoder)) {
      ...
      while (demuxer.read(readPacket) >= 0) {
        ...
        int offset = 0;
        do {
          offset += decoder.decodeAudio(input, readPacket, offset);
